...
SET_NAME = f"noML_101_2_{DATA_COLUMN}"
FOLDS = 5
#########
...
if __name__ == "__main__":
    ...
    y_train_1601 = torch.tensor(df_train_1601.values)[:, 8:9]
    x_train_401_ = torch.tensor(df_train_401.values)[:, :8]
    y_train_401 = torch.tensor(df_train_401.values)[:, 8:9]
    x_train_101_ = torch.tensor(df_train_101.values)[:, :8]
    y_train_101 = torch.tensor(df_train_101.values)[:, 8:9]
    ...
    data = TensorDataset(x_train_101, y_train_101)
    model_params_iter = create_subdictionary_iterator(model_params)
    training_params_iter = create_subdictionary_iterator(training_params)
    ...
    plot_result(path_figures=PATH_FIGURES, plot_name=SET_NAME + "_compare_", **cv_results, plot_function=plot_model_scatter,
