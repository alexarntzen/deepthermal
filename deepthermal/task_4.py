...
if __name__ == "__main__":
    ...
    # Load data
    data_train = torch.tensor(df_train.sort_values(by=["v", 't']).values)  # 0 time, 1 Temperature
    data_measured = torch.tensor(df_measured.values)  # 0 time, 1 velocity, 2 Temperature
    ...
    legend1 = ax.legend(*scatter.legend_elements(),
                        loc="upper left", title="v")
    ax.add_artist(legend1)

    plt.savefig(f"{PATH_FIGURES}/plot_task_data.pdf")
    # # Standardise values
    # # Normalize values
