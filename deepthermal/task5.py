...
def plot_task5(x_sampled, pred_sampled, x_minimizer, plot_name, path_figures=PATH_FIGURES):
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.set_xlabel("v")
    ax.set_ylabel("T")
    ax.set_title("Cost function minimizer vizualization")
    plt.scatter(x_sampled[:, 0], x_sampled[:, 1], c=pred_sampled[:, 0], label="sampled points")
    plt.scatter(x_minimizer[:, 0], x_minimizer[:, 1], color="orange", label="minimizer points")
...
if __name__ == "__main__":
    ...
    x_test = torch.tensor(x_sample)  # *DATA_SCALE[0:2] +DATA_CENTER to get physical data
    plot_kwargs1 = {"x_test": x_test,
                    "x_train": data_train[:, 0:2],
                    "y_train": data_train[:, 2:3],
                    }
    plot_result(path_figures=PATH_FIGURES, model_list=MODEL_LIST, plot_name=SET_NAME, **cv_results,
                plot_function=plot_model_scatter, function_kwargs=plot_kwargs1)

    # optimization: find the point that optimize G
    x_opt_curve = x_test.clone().detach()
    G = get_G(cv_results["models"][0][0])
    for i in range(n_samples):
        x_opt_curve[i] = argmin(G, y_0=x_opt_curve[i], verbose=False, box_constraint=True)
    g_test = G(x_test).detach()
    plot_task5(x_test, g_test, x_opt_curve, plot_name=f"{SET_NAME}_optim_points", path_figures=PATH_FIGURES)
